...
| Migration failure boundary | Immutable preflight and reversible snapshots protect all changes until the first redundant config-entry removal. The anchor is loaded and verified before removal; cleanup after the first removal is restart-resumable and creates a repair issue if incomplete. | Home Assistant has supported registry reassignment and config-entry removal APIs, but no transaction spanning them and no supported restoration of a removed entry with its original ID. | Exercise live backed-up migration and forced restart/storage-failure cases. | Home Assistant maintainer with migration review. |
...
| Account unique ID | Resolved: uses the OAuth access token's non-empty string `sub` claim. Username/email is display-only. Provisional username-based OAuth entries migrate in place without registry changes. | Home Assistant's existing Chemelex SENZ integration uses the same token-subject identity precedent. Tests cover fresh setup, duplicates, reauthentication, legacy consolidation, malformed subjects, and in-place provisional migration. | Confirm the official Chemelex Home Assistant OAuth app issues stable `sub` values across refresh and account linking. | Chemelex/OAuth owner and Home Assistant maintainer. |
...
| Official OAuth and Cloud Account Linking | Local Application Credentials use HA's built-in PKCE implementation; the abstract flow accepts a future cloud implementation. | HA supports both local Application Credentials and centrally managed Cloud Account Linking. Chemelex has stated it is ready to issue the official Client ID and Secret. | Nabu Casa/OHF must provide staging and production return URIs; Chemelex must register them and issue the official app credentials; both parties must configure and validate account linking. No official credential is claimed to exist yet. | Chemelex OAuth team and OHF/Nabu Casa. |
