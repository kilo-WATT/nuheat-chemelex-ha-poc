    ...
    *,
    ...
    serial_entry_ids = tuple(
        (
            ...
            (
                entry.entry_id
                if (entry := legacy_by_serial.get(serial)) is not None
                and entry.entry_id != anchor_entry.entry_id
                else None
            ),
        )
...
async def execute_migration_plan(
    ...
    *,
    ...
    try:
        ...
        for snapshot in plan.redundant_entry_snapshots:
            redundant_entry = hass.config_entries.async_get_entry(snapshot.entry_id)
            if redundant_entry is None:
